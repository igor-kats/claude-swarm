...
class Orchestrator:
    ...
    def plan_feature(self, feature_description: str) -> list[Task]:
        ...
        try:
            ...
            tasks = [
                Task(id=self._generate_task_id(), agent_type=AgentType.CODER,
                     description=f"Implement: {feature_description}"),
                Task(id=self._generate_task_id(), agent_type=AgentType.SECURITY,
                     description="Security review", depends_on=["task_001"]),
                Task(id=self._generate_task_id(), agent_type=AgentType.REVIEWER,
                     description="Code review", depends_on=["task_001"]),
                Task(id=self._generate_task_id(), agent_type=AgentType.TESTER,
                     description="Write tests", depends_on=["task_001"]),
            ]
