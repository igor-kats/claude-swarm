...
def status():
    ...
    try:
        ...
        for session in sessions[:10]:
            table.add_row(
                session["session_id"],
                session["feature"][:40] + "..." if len(session["feature"]) > 40 else session["feature"],
                session["status"],
                session["updated"][:19],
            )
...
@click.option("--agent", "-a", type=click.Choice([a.value for a in AgentType]), 
              default="coder", help="Agent to use")
...
def plan(feature: str, execute: bool):
    ...
    try:
        ...
        if tasks:
            ...
            console.print("[yellow]Could not generate a plan. Try a more specific description.[/yellow]")
